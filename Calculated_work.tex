...
\end{titlepage}
	...
	$$\frac{\partial^2F_{\vec\xi}^{\left(D_3\right)}}{\partial x \partial y} = \frac{\partial^2}{\partial x \partial y}\left(\frac{xy + 2\arcsin\frac{x}{2} + \frac{x}{2}\sqrt{4-x^2} - 0.25y^2 + 2y + \pi}{6+2\pi}\right) = 0 = f_{\vec\xi}
	$$
	...
	$$  F_{\vec\xi}^{\left({D_1}\right)}(x, 0) =  F_{\vec\xi}^{\left({D_3}\right)}(x, 0) =
	$$
		
	\subsection{$(x, y) \in D_4 $}
	
	$$ F_{\vec \xi}^{\left(D_4\right)}(x, y) = F_{\vec \xi}^{\left(D_1\right)}(x, 0) + \frac{1}{6+2\pi}\left(\int_{-2}^{0.5y-2}ds\int_{0}^{2s+4}dt +  \int_{0.5y-2}^{x}ds\int_{0}^{y}dt\right)= 
	$$
